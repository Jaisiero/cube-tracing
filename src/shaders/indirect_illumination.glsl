...
daxa_f32vec3 trace_hybrid_shift_rays(const daxa_b32 use_preview, const INSTANCE_HIT primary_hit, const PATH_RESERVOIR reservoir, out INSTANCE_HIT dst_rc_prev_vertex_hit, out daxa_f32vec3 dst_rc_prev_vertex_wo) {
    if(reservoir.weight == 0.0) return daxa_f32vec3(0.0);
    // TODO: calculate previus ray origin and direction?
    Ray ray;
    // = Ray(primary_hit.world_hit, primary_hit.wo);
    return trace_random_replay_path_hybrid_simple(primary_hit, ray, reservoir.path_flags, reservoir.init_random_seed, dst_rc_prev_vertex_hit, dst_rc_prev_vertex_wo);
}




void temporal_path_retrace(PATH_RESERVOIR central_reservoir, out PATH_RESERVOIR temporal_reservoir, const daxa_i32vec2 index, const daxa_u32vec2 rt_size, daxa_u32 seed, INSTANCE_HIT current_primary_hit, daxa_u32 light_count, daxa_u32 object_count, inout daxa_f32vec3 throughput) {
    ...
    // write to memory
    if (path_reservoir_get_reconnection_length(central_reservoir) > 1)
    {
        tp = trace_hybrid_shift_rays(true, temporal_first_hit, central_reservoir, dst_rc_prev_vertex_hit, dst_rc_prev_vertex_wo);
    ...
    if (path_reservoir_get_reconnection_length(temporal_reservoir) > 1)
    {
        tp2 = trace_hybrid_shift_rays(false, current_primary_hit, temporal_reservoir, dst_rc_prev_vertex_hit2, dst_rc_prev_vertex_wo2);
...
void indirect_illumination_restir_path_tracing(const daxa_i32vec2 index, const daxa_u32vec2 rt_size, Ray ray, INTERSECT i, daxa_u32 seed, daxa_u32 max_depth, daxa_u32 light_count, daxa_u32 object_count, inout daxa_f32vec3 throughput) {
    ...
    trace_restir_path_tracing(central_reservoir, index, rt_size, ray, i, seed, max_depth, light_count, object_count, throughput);
    PATH_RESERVOIR temporal_reservoir;
    temporal_path_retrace(central_reservoir, temporal_reservoir, index, rt_size, seed, i.instance_hit, light_count, object_count, throughput);
}
