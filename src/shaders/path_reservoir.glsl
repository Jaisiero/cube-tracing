...
PATH_RESERVOIR get_output_path_reservoir_by_index(daxa_u32 reservoir_index) {
  OUTPUT_PATH_RESERVOIR_BUFFER output_path_reservoir_buffer =
      OUTPUT_PATH_RESERVOIR_BUFFER(
          deref(p.restir_buffer).output_path_reservoir_address);
  return output_path_reservoir_buffer.path_reservoirs[reservoir_index];
}

void set_output_path_reservoir_by_index(daxa_u32 reservoir_index,
                                        PATH_RESERVOIR reservoir) {
  OUTPUT_PATH_RESERVOIR_BUFFER output_path_reservoir_buffer =
      OUTPUT_PATH_RESERVOIR_BUFFER(
          deref(p.restir_buffer).output_path_reservoir_address);
...
PATH_RESERVOIR get_temporal_path_reservoir_by_index(daxa_u32 reservoir_index) {
  TEMPORAL_PATH_RESERVOIR_BUFFER temporal_path_reservoir_buffer =
      TEMPORAL_PATH_RESERVOIR_BUFFER(
          deref(p.restir_buffer).temporal_path_reservoir_address);
  return temporal_path_reservoir_buffer.path_reservoirs[reservoir_index];
}

void set_temporal_path_reservoir_by_index(daxa_u32 reservoir_index,
                                          PATH_RESERVOIR reservoir) {
  TEMPORAL_PATH_RESERVOIR_BUFFER temporal_path_reservoir_buffer =
      TEMPORAL_PATH_RESERVOIR_BUFFER(
          deref(p.restir_buffer).temporal_path_reservoir_address);
...
void path_reservoir_insert_is_delta_event(inout PATH_RESERVOIR reservoir,
                                          daxa_b32 is_delta,
                                          daxa_b32 before_rc_vertex) {
  reservoir.path_flags &= (before_rc_vertex ? ~(0x100) : ~(0x200));
  if (is_delta)
    reservoir.path_flags |= 1 << (before_rc_vertex ? 20 : 21);
}

daxa_b32 path_reservoir_is_delta_event(daxa_u32 path_flags,
                                       daxa_b32 before_rc_vertex) {
  return daxa_b32((path_flags >> (before_rc_vertex ? 20 : 21)) & 1);
}

void path_reservoir_insert_is_transmission_event(inout PATH_RESERVOIR reservoir,
                                                 daxa_b32 is_transmission,
                                                 daxa_b32 before_rc_vertex) {
  reservoir.path_flags &= (before_rc_vertex ? ~(0x400) : ~(0x800));
  if (is_transmission)
    reservoir.path_flags |= 1 << (before_rc_vertex ? 22 : 23);
}

daxa_b32 path_reservoir_is_transmission_event(daxa_u32 path_flags,
                                              daxa_b32 before_rc_vertex) {
...
void path_reservoir_insert_is_specular_bounce(inout PATH_RESERVOIR reservoir,
                                              daxa_b32 is_specular_bounce,
                                              daxa_b32 before_rc_vertex) {
  reservoir.path_flags &= (before_rc_vertex ? ~(0x4000000) : ~(0x8000000));
  if (is_specular_bounce)
    reservoir.path_flags |= 1 << (before_rc_vertex ? 26 : 27);
}

daxa_b32 path_reservoir_is_specular_bounce(daxa_u32 path_flags,
                                           daxa_b32 before_rc_vertex) {
...
daxa_b32 path_reservoir_update(inout PATH_RESERVOIR reservoir,
                               daxa_f32vec3 in_F, daxa_f32 p,
                               inout daxa_u32 seed) {
  reservoir.M += 1.0;
...
daxa_b32 path_reservoir_merge(inout PATH_RESERVOIR reservoir, daxa_f32vec3 in_F,
                              daxa_f32 in_jacobian, PATH_RESERVOIR in_reservoir,
                              inout daxa_u32 seed, daxa_f32 mis_weight,
                              daxa_b32 force_add) {
  daxa_f32 w = path_F_to_scalar(in_F) * in_jacobian * in_reservoir.M *
               in_reservoir.weight * mis_weight;
...
daxa_b32 path_reservoir_merge_with_resampling_MIS(
    inout PATH_RESERVOIR reservoir, daxa_f32vec3 in_F, daxa_f32 in_jacobian,
    PATH_RESERVOIR in_reservoir, inout daxa_u32 seed, daxa_f32 mis_weight,
    daxa_b32 force_add) {
  daxa_f32 w =
      path_F_to_scalar(in_F) * in_jacobian * in_reservoir.weight * mis_weight;
...
void path_reservoir_finalize_RIS(inout PATH_RESERVOIR reservoir) {
  ...
  if (p_hat == 0.f || reservoir.M == 0.f)
    reservoir.weight = 0.f;
  else
    reservoir.weight = reservoir.weight / (p_hat * reservoir.M);
}
...
void path_reservoir_finalize_GRIS(inout PATH_RESERVOIR reservoir) {
  ...
  if (p_hat == 0.f)
    ...
    reservoir.weight = reservoir.weight / p_hat;
}