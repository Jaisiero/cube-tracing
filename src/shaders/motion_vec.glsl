...
void main() {
  ...
  // transpose(deref(p.camera_buffer).prev_inv_view); daxa_f32mat4x4
  // prev_inv_proj = transpose(deref(p.camera_buffer).prev_inv_proj);
  ...
  // daxa_f32vec4 prev_pos = prev_inv_proj * daxa_f32vec4(uv * 2.0
  // - 1.0, 1.0, 1.0);
  // // Find the world position of the current pixel
  ...
  // {
  //     velocity_buffer_set_velocity(index, p.size, VELOCITY(daxa_f32vec2(0.0,
  //     0.0))); return;
  // }
  ...
  // // deref(p.velocity_buffer).velocities[index.y * p.size.x + index.x] =
  // -motion_vector;
}