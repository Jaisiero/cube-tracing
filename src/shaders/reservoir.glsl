...
daxa_b32 is_weight_invalid(daxa_f32 weight) {
  return weight <= 0.f || isnan(weight) || isinf(weight);
}
...
void calculate_reservoir_radiance(inout RESERVOIR reservoir, Ray ray,
  ...
  if (is_reservoir_valid(reservoir)) {
    ...
    // calculate the weight of this light
    reservoir.W_y =
        p_hat > 0.0 ? (reservoir.W_sum / (reservoir.M * p_hat)) : 0.0;

    // if(p_hat == 0) initialise_reservoir(reservoir);
  }
}
...
void SPATIAL_REUSE(inout RESERVOIR reservoir, daxa_f32 confidence,
  ...
  daxa_u32 spatial_heuristic_num_of_neighbors =
      daxa_u32(mix(MAX_NUM_OF_NEIGHBORS, MIN_NUM_OF_NEIGHBORS, confidence));
