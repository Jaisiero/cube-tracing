...
void reconnection_data_initialise(out RECONNECTION_DATA reconnection_data) {
    reconnection_data.rc_prev_hit = OBJECT_HIT(OBJECT_INFO(INVALID_ID, INVALID_ID), daxa_f32vec3(0.f));
...
daxa_b32 path_is_transmission_event(PATH_STATE path) {
    return (path.flags & PATH_FLAG_TRANSMISSION) != 0;
}

daxa_b32 path_is_delta_event(PATH_STATE path) {
    return (path.flags & PATH_FLAG_DELTA) != 0;
}

daxa_b32 path_is_volume_event(PATH_STATE path) {
    return (path.flags & PATH_FLAG_VOLUME) != 0;
}

daxa_b32 path_is_inside_dielectric_volume(PATH_STATE path) {
    return (path.flags & PATH_FLAG_INSIDE_DIELECTRIC_VOLUME) != 0;
}
