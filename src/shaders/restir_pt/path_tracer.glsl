...
daxa_b32 path_generate_scatter_ray(inout PATH_STATE path, inout INTERSECT i) {
  ...
  if (valid) {
    ...
    // TODO: We classify specular events as diffuse if the roughness is above
    // some threshold.
...
daxa_b32 generate_replay_primary_scatter_ray(const SCENE_PARAMS params,
                                             inout PATH_STATE path,
                                             inout INTERSECT i,
                                             daxa_f32vec3 dir) {
...
void path_skip_light_sample_random_numbers(inout daxa_u32 seed) { rnd(seed); }

/** Evaluates the PDF for a light sample given a hit point on an emissive
   triangle. \param[in] posW Shading point in world space. \param[in] hit
   Triangle hit data. \return Probability density with respect to solid angle at
   the shading point.
*/
daxa_f32 eval_voxel_pdf(const daxa_f32vec3 pos_W, const INTERSECT i) {
  ...
  // Cosine of angle between the light's normal and the light vector (flip L
  // since it points towards the light).
  ...
  // Compute probability density with respect to solid angle from the shading
  // point. The farther away the light is and the larger the angle it is at, the
  // larger the pdf becomes. The probability goes to infinity in the limit.
...
/** Evaluate the PDF at a shading point given a hit point on an emissive
   triangle. \param[in] i Intersection data. \return Probability density with
   respect to solid angle at the shading point.
*/
daxa_f32 path_evaluate_emissive(daxa_f32vec3 origin, daxa_f32vec3 normal,
                                INTERSECT i, daxa_f32 light_count) {
...
daxa_b32 path_generate_light_sample(const SCENE_PARAMS params, INTERSECT i,
                                    const daxa_b32 sample_upper_hemisphere,
                                    const daxa_b32 sample_lower_hemisphere,
                                    inout daxa_u32 seed, out LIGHT_SAMPLER ls) {
  ...
  daxa_b32 found = sample_lights(hit, light, pdf, l_pos, l_nor, ls.Li, seed, G, false, true, true);
...
daxa_b32 path_handle_emissive_hit(const SCENE_PARAMS params,
                                  inout PATH_STATE path, INTERSECT i,
  ...
  if (!primary_hit) {
    ...
    // then we need to evaluate the MIS weight here to account for the remaining
    // contribution.
    light_pdf =
        path_evaluate_emissive(path.origin, path.normal, i, params.light_count);
  ...
  if (!path.enable_random_replay || terminate_random_replay_for_escape) {
    if (path.path_length > 1)
      path.L += Lr;
    // will change this to path.IsDelta() if ScreenSpaceReSTIR can handle
  ...
  if (!path.enable_random_replay || terminate_random_replay_for_escape) {
    ...
    daxa_b32 selected = path_builder_add_escape_vertex(
        path.path_builder, max(0, daxa_i32(path.path_length) - 1), path.dir, Lr,
        postfix_weight, path.use_hybrid_shift, path.russian_roulette_PDF,
        mis_weight, light_pdf, GEOMETRY_LIGHT_CUBE, path.path_reservoir,
        path.enable_random_replay);

    // if current escaped vertex has path.path_length >= 2, then it can be used
    // as a rcVertex
    if (selected && path.use_hybrid_shift && path.path_length >= 2 &&
        path.path_length < path.path_builder.rc_vertex_length) {
      // insert path flags

      //  ideally, this should use a smaller nearFieldDistance than the default
      //  threshold
      daxa_b32 is_far_field =
          length(i.world_hit - path.origin) >= params.near_field_distance;
      ...
      if (!(!is_far_field || !is_last_vertex_acceptable_for_rc_prev)) {
        if (path.is_replay_for_hybrid_shift) // non-invertible case
        {
          ...
          path_builder_mark_escape_vertex_as_rc_vertex(
              ...
              path_is_delta_event(path), path_is_transmission_event(path),
...
daxa_b32 path_handle_sample_light(const SCENE_PARAMS params,
                                  inout PATH_STATE path, INTERSECT i,
                                  daxa_b32 terminate_random_replay_for_NEE,
                                  daxa_b32 is_rc_vertex) {

  daxa_b32 valid_sample = false;
  // if we are doing random number replay, we should only sample the light if we
  // are about to terminate

  // generate kNEESamples light samples
  if ((!path.enable_random_replay || terminate_random_replay_for_NEE)) {
    // cache (NEE sampling) seed for temporal validation (only possible for non
    // early direction reuse since it uses the cachedRandomSeed slot)
    if (path.path_length == path.path_builder.rc_vertex_length) {
    ...
    LIGHT_SAMPLER ls;
    {
      // Setup path vertex.
      PATH_VERTEX vertex =
          PATH_VERTEX(path.path_length + 1, i.world_hit, i.world_nrm);

      // TODO: complete this function
      // // Determine if upper/lower hemispheres need to be sampled.
      // daxa_b32 sample_upper_hemisphere = ((lobes &
      // (uint)LobeType::NonDeltaReflection) != 0); if
      // (!kUseLightsInDielectricVolumes && path.isInsideDielectricVolume())
      //     sample_upper_hemisphere = false;
      // daxa_b32 sample_lower_hemisphere = ((lobes &
      // (uint)LobeType::NonDeltaTransmission) != 0);
      ...
      valid_sample =
          path_generate_light_sample(params, i, sample_upper_hemisphere,
                                     sample_lower_hemisphere, path.seed, ls);

      path_set_light_sampled(path, sample_upper_hemisphere,
                             sample_lower_hemisphere);
    }
    ...
    if (valid_sample) {
      ...
      // if we have enabled random replay, we know that we are going to use this
      // path
      if (is_rc_vertex) {
        weight = daxa_f32vec3(1.f);
      }

      daxa_b32 selected = path_builder_add_NEE_vertex(
          path.path_builder, path.path_length, ls.dir, Lr,
          weight * ls.Li * path.thp, path.use_hybrid_shift,
          path.russian_roulette_PDF, mis_weight, ls.pdf, ls.type,
          path.path_reservoir, path.enable_random_replay);
    }
  } else
  // for random number replay, we should skip the random numbers for NEE if we
  // should not terminate on current bounce
  {
...
daxa_b32 path_check_rc_vertex(inout PATH_STATE path, INTERSECT i,
                              daxa_f32vec3 prev_path_origin,
                              daxa_b32 is_far_field,
                              daxa_b32 is_current_vertex_classified_as_rough,
                              daxa_b32 is_last_vertex_acceptable_for_rc_prev,
                              out daxa_b32 is_rc_vertex) {
  //  if we are not doing a hybrid shift replay, we should check if current
  //  vertex satisfy the condition to be used as a rcVertex
  if (!path.is_replay_for_hybrid_shift) {
    daxa_b32 can_connect =
        (path.path_length >= 1 &&
         path.path_length < path.path_builder.rc_vertex_length &&
         (!(!is_far_field || !(is_current_vertex_classified_as_rough &&
                               is_last_vertex_acceptable_for_rc_prev))));

    if (!path.use_hybrid_shift && path.path_length == 1 ||
        path.use_hybrid_shift && can_connect) {
      is_rc_vertex = true;
    ...
    if (path.path_length >= 1 &&
      ...
      //     {
      //         // method 2
      //         // non-invertible if this bounce is an NEE bounce for the base
      //         path but the current vertex is not connectible in base path but
      //         is connectible in offset path if
      //         (terminate_random_replay_for_NEE &&
      //         isCurrentVertexClassifiedAsRough &&
      //         is_last_vertex_acceptable_for_rc_prev)
      //             invertible = false;
      //         // non-invertible because current vertex will be assigned as a
      //         diffuse bounce by connecting to the reconnection vertex if
      //         (path.path_length == path.pathBuilder.rcVertexLength - 1 &&
      //         is_last_vertex_acceptable_for_rc_prev)
      //             invertible = false;
      ...
      //     {
      //         if (params.localStrategyType &
      //         (uint)LocalStrategy::DistanceCondition && is_far_field)
      //             invertible = false;
      //     }
      // }
      // else
      {
        if (!is_far_field || !(is_current_vertex_classified_as_rough &&
                               is_last_vertex_acceptable_for_rc_prev))
          invertible = false;
      }
    }

    // if next vertex is rcVertex and the shift is still invertible, then we
    // should terminate
    if (path.path_length == path.path_builder.rc_vertex_length - 1 &&
        invertible) {
      ...
      // TODO: near field rejection case is checked in
      // computeShiftedIntegrandReconnection in shift.slang
      else {
    ...
    if (invertible) {
      // if it is invertible and we should terminate, save the hit information
      // to set up a reconnection
      if (should_terminate) {
        path.rc_prev_vertex_hit = path.hit;
        path.rc_prev_vertex_wo = i.wo;
        path_terminate(path);
      }
    } else // if the shift is not invertible, we should mark the case, which
           // causes the shift to terminate when it goes back to shift.slang
    {
...
void path_handle_hit(const SCENE_PARAMS params, inout PATH_STATE path,
  ...
  if (path.path_length == 1 && !path_is_transmission_event(path) &&
      !path_is_delta_event(path))
    compute_emissive = false;

  //  when doing random number replay, we will terminate the path when the
  //  length reaches the base path length and
  // when the path types match. In this case, if the base path is a escaped path
  // (as opposed to an NEE path), we should terminate when we find that offset
  // path can also be an escaped path
  daxa_b32 terminate_random_replay_for_escape =
      path.enable_random_replay &&
      path.path_length - 1 == path.random_replay_length &&
      path.random_replay_is_escaped && path.random_replay_length >= 1;

  // Sample emissive (add escape vertex) (pathtracer:1034)
  if (compute_emissive && any(greaterThan(i.mat.emission, daxa_f32vec3(0.f)))) {
    path_handle_emissive_hit(params, path, i,
                             terminate_random_replay_for_escape);
  }

  // Terminate after scatter ray on last vertex has been processed.
  if (path_has_finished_surface_bounces(path) ||
      terminate_random_replay_for_escape) {
    path_terminate(path);
    return;
  }
  ...
  // when doing random number replay, we will terminate the path when the length
  // reaches the base path length and when the path types match. In this case,
  // if the base path is a NEE path (as opposed to an escaped path), we should
  // terminate when we find that offset path can also be an NEE path
  daxa_b32 terminate_random_replay_for_NEE =
      path.enable_random_replay &&
  ...
  if (!path_check_rc_vertex(path, i, prev_path_origin, is_far_field,
  ...
  path_handle_sample_light(params, path, i, terminate_random_replay_for_NEE,
                           is_rc_vertex);

  //  terminate the path if the bounce of random replay reaches the desired
  //  bounce and the base path is an NEE path
  ...
  // TODO: forbid current vertex being rcVertex if conditions are not met
  // (pathtracer:1403)

  // cache seed for temporal validation (only possible for non early direction
  // reuse since it uses the cachedRandomSeed slot) (pathtracer:1427)
  if (path.path_length == path.path_builder.rc_vertex_length) {
    path.path_builder.cached_random_seed = path.seed;
  }
  ...
  // TODO: Check other things to invalidate the path (caustics)
  // (pathtracer:1465)
...
void path_handle_miss(const SCENE_PARAMS params, inout PATH_STATE path,
                      inout INTERSECT i) {
  // Upon miss:
  ...
  if (params.compute_environment_light) {
    ...
    // Note we can assume path.pdf > 0.f since we shouldn't have got here
    // otherwise.
    ...
    if ((!path.enable_random_replay || terminate_random_replay_for_escape)) {
      daxa_b32 selected_current_path = path_builder_add_escape_vertex(
          ...
          path.thp * Le * mis_weight, path.use_hybrid_shift,
      ...
      if (selected_current_path && path.use_hybrid_shift &&
          path.path_length >= 1 &&
          path.path_length + 1 < path.path_builder.rc_vertex_length) {
        daxa_b32 is_last_vertex_acceptable_for_rc_prev =
            path.is_last_vertex_classified_as_rough;
        ...
        if (!(!is_last_vertex_acceptable_for_rc_prev)) {
          if (path.is_replay_for_hybrid_shift) // non-invertible case
          {
            ...
            path_builder_mark_escape_vertex_as_rc_vertex(
                ...
                path_is_transmission_event(path), path_is_specular_bounce(path),
                light_pdf, GEOMETRY_LIGHT_ENV_MAP, Le, path.dir,
...
PATH_RESERVOIR trace_restir_path_tracing(const SCENE_PARAMS params,
                                         const daxa_i32vec2 index,
                                         const daxa_u32vec2 rt_size, Ray ray,
...
daxa_f32vec3 trace_random_replay_path_hybrid_simple(
  ...
  path_flags_transform_bounces_information(path.path_builder.path_flags,
                                           path_flags);
  ...
  } else // this branch is the case where rcVertex does not exist (or infinitely
         // far)
  {
    L = path.L;
  }
...
void path_tracer_trace_temporal_update(const INTERSECT dst_primary_intersection,
